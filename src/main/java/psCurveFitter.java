 ...
 * (3-parameter) fit types. 2013-10-11: bugfixes, added setStatusAndEsc to show
 * iterations and enable abort by ESC 2015-03-26: bugfix, did not use linear
 * regression for RODBARD 2016-11-28: added static getNumParams methods
 * 2018-03-23: fixes NullPointerException for custom fit without
 * initialParamVariations
 */

public class psCurveFitter implements UserFunction {
	...
	public psCurveFitter(double[] xData, double[] yData) {
		this.xData = xData;
		this.yData = yData;
	...
	public void doFit(int fitType, boolean showSettings) {
		...
		if (this.fitType == STRAIGHT_LINE) { // no minimizer needed
			...
			//minimizer.setFunction(this, 2);
			// IJ.log(Integer.toString(numParams)+" "+Integer.toString(numRegressionParams));
	 ...
	 */
	public void doCustomFit(UserFunction userFunction, int numParams, String formula, double[] initialParams,
	 ...
	 */
	public final double fitter(double x) {
		if (finalParams == null)
			finalParams = minimizer.getParams();
		return fitter(finalParams, x);
	}
	 ...
	 * 'doFit', because the fit function would be undefined.
	 */
	public final double fitter(double[] p, double x) {
		if (fitType != CUSTOM)
			return fitter(fitType, p, x);
		else {
	 ...
	 */
	public static double fitter(int fitType, double[] p, double x) {
		switch (fitType) {
			case STRAIGHT_LINE:
				return p[0] + x * p[1];
			case POLY2:
			...
			case RODBARD2: // also used after the 'Rodbard NIH Image' fit
				...
				return y;
			case EXP2FIT:
				double valueExp2Fit = p[0] * (Math.exp(-p[1] * x)) + p[2] * (Math.exp(-p[3] * x)) + p[4];
				// Log.e("test", "values = " + value);
				return valueExp2Fit;

			case EXP1FIXFIT:
				return p[0]+Math.pow(p[1],2) *(Math.exp(-4.3 * x));
	...
	public double[] getResiduals() {
		...
		for (int i = 0; i < xData.length; i++)
			residuals[i] = yData[i] - fitter(params, xData[i]);
		return residuals;
	}
	...
	public final double userFunction(double[] params, double dummy) {
		...
		if (numRegressionParams == 0) { // simply calculate sum of residuals
			for (int i = 0; i < numPoints; i++) {
				double fValue = fitter(params, xData[i]);
	...
	private void doRegression(double[] params) {
		...
		for (int i = 0; i < numPoints; i++) {
			double fValue = fitType == STRAIGHT_LINE ? 0 : fitter(params, xData[i]); // function value
		...
		if (offsetParam < 0) { // only 'factor' parameter, no offset:
			...
			sumResidualsSqr = sqr(factor) * sumX2 + numPoints * sqr(offset) + sumY2 + 2 * factor * offset * sumX
					- 2 * factor * sumXY - 2 * offset * sumY;
	...
	private boolean makeInitialParamsAndVariations(int fitType) {
		...
		if (!hasInitialParams) {
			switch (fitType) {
				...
				case LOG2: // y = a+b*ln(x-c)
					initialParams[0] = yMean; // don't care, we will do this via regression
					initialParams[1] = (yMax - yMin) / (xMax - xMin + 1e-100); // don't care, we will do this via
																				// regression
				...
				case EXP2FIT:
					...
					initialParams[0] = (yMax - yMin) / Math.exp(initialParams[1] * xMean) * Math.signum(slope)
							* fitType == EXP_RECOVERY ? 1 : -1; // don't care, we will do this via regression
					initialParams[2] = 1. / (xMax - xMin + 1e-100);
					initialParams[3] = (yMax - yMin) / Math.exp(initialParams[1] * xMean) * Math.signum(slope)
							* fitType == EXP_RECOVERY ? 1 : -1; // don't care, we will do this via regression
					initialParams[4] = 0.5 * yMean; // don't care, we will do this via regression
					break;
		...
		if (!hasInitialParamVariations) { // estimate initial range for parameters
			...
			switch (fitType) {
				...
				case EXP2FIT:
					initialParamVariations[1] = 0.1 / (xMax - xMin + 1e-100);
					initialParamVariations[3] = 0.1 / (xMax - xMin + 1e-100);
					break;
				case EXP1FIXFIT:
					initialParamVariations[1] = 0.1 / (xMax - xMin + 1e-100);
					break;
	...
	private boolean isModifiedFitType(int fitType) {
		return fitType == POWER_REGRESSION || fitType == EXP_REGRESSION || fitType == RODBARD || fitType == RODBARD2
				|| fitType == GAUSSIAN;
	}
